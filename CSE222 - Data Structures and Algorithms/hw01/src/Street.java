...
public class Street {
	...
	*/
	...
	public Street(int streetSize) throws IOException {
	...
	*/
	...
	private String decider(int row, int column){
		...
		if ( str == "|") {		// If a building has a higher building next to it (to front or back), then outline (walls) of that exact part is not printed
		...
		if ( str == "_") {		// same with "line 312" but for roofs
		...
		if (count > 1)
			if ( str == "|") str = " ";	// outlines exactly match then no character is printed for a expected result
			else if ( str == "_") str = "|";
